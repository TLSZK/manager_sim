      ...
      <div className="bg-slate-900 w-full h-[90vh] md:h-auto md:max-h-[85vh] md:max-w-5xl md:rounded-2xl border-t md:border border-slate-700 shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-3 md:p-4 bg-slate-800 border-b border-slate-700 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2 md:gap-4 flex-1">
             <button onClick={() => changeMonth(-1)} className="p-1.5 md:p-2 hover:bg-slate-700 rounded-full transition-colors text-slate-300"><ChevronLeft size={20} /></button>
             <h2 className="text-lg md:text-xl font-bold text-white text-center flex-1 md:flex-none md:min-w-[150px]">{monthName}</h2>
             <button onClick={() => changeMonth(1)} className="p-1.5 md:p-2 hover:bg-slate-700 rounded-full transition-colors text-slate-300"><ChevronRight size={20} /></button>
             <button onClick={jumpToCurrent} className="ml-auto md:ml-4 flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs md:text-sm font-bold rounded-lg transition-colors whitespace-nowrap"><CalendarIcon size={14} /> <span className="hidden sm:inline">Current</span></button>
          </div>
          <button onClick={onClose} className="p-2 ml-2 hover:bg-slate-700 rounded-full text-slate-400 hover:text-white"><X size={24} /></button>
        </div>
        ...
        <div className="flex-1 p-1 md:p-6 overflow-y-auto bg-slate-950/50">
          <div className="grid grid-cols-7 gap-px bg-slate-700 border border-slate-700 rounded-lg overflow-hidden">
             {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (<div key={d} className="bg-slate-800 p-2 md:p-3 text-center text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider"><span className="md:hidden">{d.charAt(0)}</span><span className="hidden md:inline">{d}</span></div>))}
             {daysArray.map((day, idx) => {
                 ...
                 return (
                     <div key={idx} className={`bg-slate-900 min-h-[60px] md:min-h-[100px] p-1 md:p-2 relative group border-t border-l border-slate-800 hover:bg-slate-800/50 transition-colors ${isSimCurrentDate ? 'bg-indigo-900/20' : ''}`}>
                         <div className="flex justify-between items-start"><span className={`text-xs md:text-sm font-bold ${isSimCurrentDate ? 'text-indigo-400' : 'text-slate-500'}`}>{day}</span></div>
                         {match && opponent && (
                             <div className={`mt-1 md:mt-2 p-1 md:p-2 rounded md:rounded-lg text-[9px] md:text-xs border cursor-pointer relative overflow-hidden group/match shadow-md ${isUCL ? 'bg-blue-900/30 border-blue-700/50 hover:bg-blue-900/50' : 'bg-emerald-900/20 border-emerald-700/50 hover:bg-emerald-900/40'}`} onClick={() => canSimTo && onSimulateToWeek(match.week)}>
                                 <div className="flex flex-col md:flex-row items-start md:items-center gap-1 md:gap-1.5 mb-0.5 md:mb-1">
                                     <div className="flex items-center gap-1">
                                         ...
                                         <span className={`font-mono font-bold text-[8px] md:text-[9px] px-1 rounded ${isHome ? 'bg-white/10 text-white' : 'bg-black/20 text-slate-400'}`}>{isHome ? 'H' : 'A'}</span>
                                     </div>
                                     <span className={`font-bold truncate max-w-full ${isUCL ? 'text-blue-200' : 'text-emerald-200'}`}>{opponent.shortName}</span>
                                 </div>
                                 {isPlayed ? (<div className="font-mono font-bold text-white bg-black/40 text-center rounded py-0.5 md:py-1">{match.homeScore} - {match.awayScore}</div>) : (<div className="text-center text-slate-400 text-[8px] md:text-[10px] uppercase font-bold tracking-wider bg-black/20 rounded py-0.5">{match.week < currentWeek ? 'TBP' : 'VS'}</div>)}
                                 {canSimTo && (<div className="absolute inset-0 bg-black/80 flex items-center justify-center opacity-0 group-hover/match:opacity-100 transition-opacity"><span className="flex items-center gap-1 text-[10px] font-bold text-white uppercase tracking-wider"><PlayCircle size={14} /> Sim</span></div>)}
                             </div>
                         )}
                     </div>
                 );
